...
public extension UICollectionView {
    ...
    /// SwifterSwift: Scroll to top of CollectionView.
    ///
    ...
    func register<T: UICollectionReusableView>(forSupplementaryViewOfKind kind: String, withNib name: T.Type) {
        let nib = UINib(nibName: String(describing: name), bundle: Bundle.main)
        register(nib, forSupplementaryViewOfKind: kind, withReuseIdentifier: String(describing: name))
    }
    ...
    /// - Parameters:
    ///   - name: UICollectionViewCell type.
    func register<T: UICollectionViewCell>(xibWithCellClass name: T.Type) {
    ...
    func register<T: UICollectionReusableView>(forKind kind: String, withNibClass name: T.Type) {
        let info = NSStringFromClass(T.self).separateTypeName()
        guard let module = info.module, let identifier = info.file else {fatalError("module or idenitifer is nil")}
        guard let bundle = Bundle.create(module: module) else {fatalError("bundle is nil")}
