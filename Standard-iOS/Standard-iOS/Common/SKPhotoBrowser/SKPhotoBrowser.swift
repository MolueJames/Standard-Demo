...
open class SKPhotoBrowser: UIViewController {
    ...
    private var isBlackBarStyle: Bool = false
    // appearance
    ...
    open override var preferredStatusBarStyle: UIStatusBarStyle {
        get { return self.isBlackBarStyle ? .default : .lightContent }
    }
    ...
    open func popupShare(includeCaption: Bool = true) {
        func setNeedsStatusBarAppearance(with flag: Bool) {
            self.isBlackBarStyle = flag
        ...
        activityViewController.completionWithItemsHandler = { (activity, success, items, error) in
            ...
            setNeedsStatusBarAppearance(with: false)
        }
        ...
        present(activityViewController, animated: true, completion: {
            setNeedsStatusBarAppearance(with: true)
        })
    }
}
    ...
    @objc func panGestureRecognized(_ sender: UIPanGestureRecognizer) {
        ...
        if sender.state == .ended {
            ...
            if zoomingScrollView.center.y > viewHalfHeight + minOffset
                ...
                UIView.animate(withDuration: animationDuration, delay: 0, usingSpringWithDamping: 0, initialSpringVelocity: 0, options: .curveEaseIn, animations: {
